...
exname = '2022_R4'
Nphi = 100          # S3経度の分割数
rho0_best = 1708.74447204   # [amu cm-3]
Ti_best = 195.68800819      # [eV]
# rho0_best = 1940.47505231
# Ti_best = 163.67050013
...
def BEST_FIT(rho0_best: float, HP_best: float, TIME=False):
    ...
    np.savetxt('img/LeadangleFit/'+exname+'/bestfit.txt',
    ...
    if TIME:
        savetag = 'time'
        ax.set_ylabel('Travel time [min]', fontsize=fontsize)
        ax.set_ylim(-1.05, 30)      # [minute]
        ...
        for i in range(len(doy1422)):
            deg2min = 1/(60*OMGR*180/np.pi)
            data_arr = np.loadtxt(
                'data/red3_leadangle/EUROPA/20'+doy1422[i]+'_eq.txt')
            sc = ax.scatter(data_arr[0, :], data_arr[1, :]*deg2min,
                            marker=',', s=0.9, c=cud4[0], linewidths=0.65, zorder=1)
            ax.errorbar(data_arr[0, :], data_arr[1, :]*deg2min,
                        yerr=la_err[i]*deg2min*np.ones(data_arr[0, :].shape),
                        linestyle='none', ecolor=cud4[0], elinewidth=0.22, marker='none', zorder=1)
            if i == 0:
                sc.set_label('HST')

        # Bestfit
        ax.plot(moonS3wlon, eqlead_est*deg2min,
    ...
    plt.savefig('img/LeadangleFit/'+exname +
                '/bestfit_'+savetag+'.png', bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    BEST_FIT(rho0_best, HP_best, TIME=False)
